...
2.  Installez les dépendances nécessaires via le terminal de VS Code :
    ```bash
    pip install numpy pandas scikit-learn
    ```
3.  Placez le fichier `diabetes.csv` dans le même dossier que votre script.

## ⚙️ Étapes du Code
1.  **Analyse des données** : Chargement du CSV et statistiques descriptives avec `.describe()`.
2.  **Prétraitement** : Normalisation des données avec `StandardScaler` pour que toutes les mesures soient sur la même échelle.
3.  **Entraînement** : Utilisation d'un noyau linéaire (`kernel='linear'`) avec l'algorithme SVM.
4.  **Évaluation** : Calcul du score de précision (Accuracy) sur les données d'entraînement et de test.
